...
\author{Dávid Márk Nemeskey\inst{1} \and Francis M. Tyers\inst{2}}

\institute{ HAS Computer and Automation Research Institute,\\
Kende u 13-17,\\
H-1111 Budapest\\
\email{nemeskey.david@sztaki.mta.hu}
\and Dept. Lleng. i Sist. Inform.,\\
...
In Sect.~\ref{sec:fomacg} briefly introduces the method we chose to evaluate.
...
a sentence that consists of \emph{cohorts}: a word with its possible meanings.
A meaning (or \emph{reading} in CG parlance) is represented by a set of
morphosyntactic \emph{tags}. For example, the cohort of the ambiguous Hungarian
word \textit{szív} with two readings "heart" and "to suck" would be \\
\texttt{\^{}szív/szív<n><sg><nom>/szív<vblex><pres><s3p>\$}\footnote{
The example is in the Apertium stream format, not in CG-2 style.}. CG rules
operate by removing readings based on the context of the word. Most
...
\emph{fomacg} is a only compiler, which reads a CG rule file and emits a
\emph{foma} FST for each rule. The actual disambiguator program that applies
the transducers to text we had to implement ourselves.
...
Internally \texttt{foma}, similarly to other FST toolkits, represents elements
...
\texttt{foma}, token matching is performed by a trie built from the symbols in
...
%format, the symbols are separated by space characters. Instead of passing the
...
%grammar improved by 0.4 second.

The second enhancement aims to negate the cost of token conversion.
It came from the observation that all rule testing automata
...
Both \texttt{fomacg} and \texttt{fomacg-proc} had to be modified for to add the
converter FSA. The former now creates the converter FSA and saves it as the
second machine in the binary
grammar file. Also, since the ids that correspond to a symbol are unique to
each machine, we added a postprocessing phase that replaces the ids with the
"canonical" ones in the converter FSA. 
\texttt{fomacg-proc} then converts the input to ids using the converter
...
Hungarian setup improved by 70\% to 0.32 second; the Breton one by \% to second.  %TODO
...
\begin{table}[h]
  ...
  Custom rule applier (\ref{sec:speed_cg})      & 1.05s  &       \\
%  Input partitioning (\ref{sec:speed_cg})       & 1.45s  &       \\
...
Hulden~\cite{Hulden:2011} claims that if the rules are compiled to FSTs,
...
the size of the automata in their roots.  % TODO FSAs?
...
% TODO: rewrite these two paragraphs (tries have already been introduced)
...
requirements of the grammar as well.
...
% TODO describe the trie (e.g. byte-based, 256-way (x pointer), etc.)
The tries that convert the symbols to integer ids in \texttt{foma}
...
XXX et al.~\cite{XXX:2014} present an algorithm for lossless
