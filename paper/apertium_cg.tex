...
This work have improved on two aspects of the complexity, however. First, the
...
sentence and $G_u$ cannot, $G_a + G_u = G$. With deterministic automata for
rule testing the round finishes in $\mathcal{O}(nkG + n^2kG_a)$. Usually
$G_a << G$, and the complexity converges to $\mathcal{O}(nkG)$; in the best
case, when no rule is applicable, the whole disambiguation process finishes in
that time.
...
\subsection{Beyond the $\mathcal{O}(n^3k^2G)$ Bound}
\label{sec:idea}

Here we present an idea, due to Mans Hulden, that allows the system to
theoretically overcome the $\mathcal{O}(n^3k^2G)$ complexity bound. In this
section, we first describe the method, then evaluate it on our grammars and
...
Automata Mista, due to Huet~\shortcite{Huet:2004,Huet:2005}, is an applicative
