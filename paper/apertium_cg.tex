...
\begin{table}[h]
  \centering
  \caption{Effects of the optimizations on the running time}
  \label{tab:trees}
  \begin{tabular}{ | l | r | r | }
  \hline
  \textbf{Section} & \textbf{Hungarian} & \textbf{Breton} \\
  \hline
  \ref{sec:speed_naive}   &  &  \\
  \ref{sec:speed_deletex} &  &  \\
  \ref{sec:speed_custom}  &  &  \\
  \ref{sec:speed_convert} &  &  \\
  \hline
  \end{tabular}
\end{table}
...
\begin{table*}[h]
  ...
  Breton    & FixedLevel(2) & 2.278s & 1.49s & 9.6\% & 7.4MB \\
